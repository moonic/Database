...
* mongoDB 的易用性，分布式环境的单点下性能的海量数据支持的要求没有
那么重要,要找到易用的

SourceForge execute mongodb save Auto Sharding 功能升级了自由水平的
架构方案，是目前最火的NOSQL系统

但在系统维护方面缺少业界的实践，只能靠程序员自己摸索
...
为什么要使用非关系型数据库
	...
	集群的大量数据要更改模式，时间很长DBA投入时间，可以可以在核心服	   务器内置更多的此类功能
	MonogoDB 的实用性，他在功能性和复杂性取得更好的平衡支持现在Web
	  index copy select language


MongoDB是一种强大灵活可扩展的数据存储方式 扩展了非关系数据库的
	众多功能 比如辅助索引 访问查询(range query) 和排序，比如内置的
	对MapReduce 的聚合式支持，已经对物理空空间的索引支持

	容易操作有shell提供的自然语言的APL 关注程序本身不是为了存储数据
		的烦恼
...
* 丰富 的数据模型
	...
	没有陌生的更改，不会需要迁移大量的数据应用层可以处理丢失的key
		开发者可以容易的变更数据类型
...
* index 
		...
		支持通用辅助索引，可以进行快速的查询也提过唯一的复合和地理空间索引能力

* save javascript 

	可以直接在服务器存储 javasc function type
